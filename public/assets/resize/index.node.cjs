...
async function init(input) {
  if (typeof input === "undefined") {
    input = new URL("squoosh_resize_bg.wasm", "file:///home/bnowak/repos/squoosh-kit/packages/resize/wasm/squoosh_resize.js");
  }
  const imports = {};
...
module.exports = __toCommonJS(exports_resize_worker);
async function init2() {
  ...
  initPromise = (async () => {
    try {
      const workerBaseUrl = new URL(".", "file:///home/bnowak/repos/squoosh-kit/packages/resize/src/resize.worker.ts");
      const isSource = "file:///home/bnowak/repos/squoosh-kit/packages/resize/src/resize.worker.ts".includes("/src/");
...
class ResizeClientBridge {
  async resize(image, options, signal) {
    const module2 = await Promise.resolve().then(() => (init_resize_worker(), exports_resize_worker));
    const resizeClient2 = module2.resizeClient;
...
//# debugId=3A1DA2BD76BD939D64756E2164756E21
