...
export async function splitFile(
  ...
  switch (type) {
    case 'cbz':
      return splitCbzFile(file, ranges, outputFormat, onProgress, dimensions)
    case 'pdf':
      return splitPdfFile(file, ranges, outputFormat, onProgress, dimensions)
    case 'xtc':
      return splitXtcFile(file, ranges, outputFormat, onProgress, dimensions)
    default:
      throw new Error('Unsupported file type')
  }
}
...
function buildXtcFromRawPages(
  ...
  for (let i = 0; i < pageCount; i++) {
    ...
    view.setUint16(entryOffset + 12, targetWidth, true)
    view.setUint16(entryOffset + 14, targetHeight, true)
