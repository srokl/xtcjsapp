...
function processCanvasAsImage(
  ...
  if (isSingleImage) {
    ...
    switch (options.imageMode) {
      case 'fill':
        finalCanvas = resizeFill(processingCanvas, dims.width, dims.height)
        break
      case 'cover':
        finalCanvas = resizeCover(processingCanvas, dims.width, dims.height)
        break
      case 'crop':
        finalCanvas = resizeCrop(processingCanvas, dims.width, dims.height)
        break
      case 'letterbox':
      default:
        finalCanvas = resizeWithPadding(processingCanvas, padColor, dims.width, dims.height)
        break
    }
  ...
  if (options.manhwa) {
    ...
    const overlapPixels = Math.floor(dims.height * (overlapPercent / 100))
    const sliceStep = dims.height - overlapPixels
    ...
    for (let y = 0; y < newHeight; ) {
      ...
      const sliceCanvas = extractRegion(resizedCanvas, 0, y, dims.width, h)
      const finalCanvas = resizeWithPadding(sliceCanvas, padColor, dims.width, dims.height)
